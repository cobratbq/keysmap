...
GNUPG_LOCAL=gpg --no-default-keyring --keyring $(KEYRING)
...
pgp-keys.map: tools artifact-signatures $(KEYRING)
...
$(KEYRING): tools artifact-signatures
...
.PHONY: distclean
distclean: clean
	rm -rf artifact-metadata artifact-signatures signatures $(KEYRING){,~}
	$(MAKE) -C tools clean
